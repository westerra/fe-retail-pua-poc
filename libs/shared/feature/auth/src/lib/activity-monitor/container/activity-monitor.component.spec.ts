...
describe('ActivityMonitorComponent', () => {
  const getInstance = () => {
    ...
    const activityMonitorService = jasmine.createSpyObj<ActivityMonitorService>('ActivityMonitorService', [
      'start',
      'stop',
    ]);
    // eslint-disable-next-line  @typescript-eslint/no-explicit-any
    (<any>activityMonitorService).events = activityEvents;

    const oAuthService = jasmine.createSpyObj<OAuthService>('OAuthService', [
      'hasValidAccessToken',
      'logOut',
      'initLoginFlow',
    ]);
    ...
    const component = new ActivityMonitorComponent(activityMonitorService, oAuthService, authService);
  ...
  describe('#logoutOnEndEvent', () => {
    it('should logout user when end event is emitted and user has valid access token', async () => {
      const { component, scheduler, activityEvents, oAuthService } = getInstance();
      const { endEvent } = getData();
  ...
  describe('#startActivityMonitorWhenAuthenticated', () => {
    it('should start activity monitor when the user is authenticated', async () => {
      const { component, scheduler, isAuthenticated$, activityMonitorService } = getInstance();
